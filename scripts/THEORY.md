...
2. **方向正确但强度不足** — 正负词语投影方向符合预期，但区分度较低
3. **pca-align 提升明显** — 相似度和区分度都有显著提升，但可能引入人为偏差
4. **单词对噪声大** — 单对词定义概念轴不稳定，需多词对平均改进
5. **真值轴高度一致** — "对-错"与"是-否"相似度达 0.93（pca-align），说明模型对真值判断有统一的表示
6. **好-坏与对-错不相关** — 价值判断与真值判断在语义空间中分离
