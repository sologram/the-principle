...
语用信息（Pragmatic Information）——表示概念在评价、判断和价值体系中的位置，例如：

- 好 / 坏
- 对 / 错
- 善 / 恶
  
语义信息（Semantic Information）——表示概念本身的内容、关系和指称。

此外，词向量还包含一定程度的语言形式信息（语法、词形等）。

但本文主要关注语义与语用的分离问题。

由于不同信息混合在同一个向量空间中，会产生一个反直觉现象：

语义相反的词，在向量空间中可能表现出较高的相似度。
